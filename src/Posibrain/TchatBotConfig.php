	...
	public function __construct($id = '', $lang = '', $params = array())
	{
		...
		while (! is_file($this->getKnowledgeFile())) {
			...
			if (1 == $tryNumber) {
				...
				self::$logger->addWarning('No such bot: load crazy stupid ' . $this->id . ' (' . $this->lang . ')', array(
					$id,
					$lang,
					$params
				));
			...
			if (2 == $tryNumber) {
				...
				$this->setCharset($defaultConfig['charset']);
				self::$logger->addWarning('Still no such bot: load crazy stupid ' . $this->id . ' (' . $this->lang . ')', array(
					$id,
					$lang,
					$params
				));
			...
			// Arg!
				...
				self::$logger->addError('Definitely no bot: this tchat is going to be very stupid!', array(
					$id,
					$lang,
					$params
				));
